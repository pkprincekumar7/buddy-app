...
flowchart TD
    ...
    subgraph VPC["VPC — ap-south-1  ·  3 AZs  ·  private subnets  ·  NAT GW × 3  ·  Interface Endpoints × 6 (incl. Atlas PrivateLink)  ·  S3 Gateway Endpoint × 1"]
        ALB[ALB\nHTTPS 443]
...
|---|---|---|
| 1 million users (baseline, 5 API + 2 worker tasks) | ~$2,307 | ~$27,684 |
| 1 million users (peak burst, 20 API + 10 worker tasks sustained all month) | ~$3,656 | — |
| 10 million users (baseline, 20 API + 10 worker tasks avg) | ~$4,787 | ~$57,444 |
...
| Service | 1M users/mo | 10M users/mo |
|---|---|---|
| Primary cluster M50 (64 GB RAM, ~3K connections; hard cap — do not upgrade beyond M50) | $749 | $749 |
| **Subtotal** | **$749** | **$749** |
...
| Service | 1M users/mo | 10M users/mo |
|---|---|---|
| Redis cluster mode — 3 shards × 2 nodes (m7g.large). Shard count cannot be changed in-place — requires destroy + create. Scale to 6 shards before reaching 10M users by scheduling a maintenance window. 10M cost assumes 6 shards × 2 nodes. | $726 | $1,451 |
| **Subtotal** | **$726** | **$1,451** |
...
| KMS key + rotation (CloudTrail log encryption) | $1 | $2 |
| **Subtotal** | **$60** | **$168** |
...
| SNS — alerts topic + email + PagerDuty subscriptions | $2 | $2 |
...
| S3 — regional logging bucket (ALB logs, VPC Flow Logs, CloudTrail ap-south-1) | $5 | $15 |
...
| `aws_ecs_task_definition` (API) | change | Add ADOT sidecar container for X-Ray tracing; add `REDIS_AUTH_TOKEN` secret. **ADOT sidecar spec:** container name `aws-otel-collector`; image `public.ecr.aws/aws-observability/aws-otel-collector:v0.40.0` (pin a specific tag — do not use `latest`); command `["--config=/etc/ecs/ecs-default-config.yaml"]` (uses the AWS-managed ECS default config which exports X-Ray traces and CloudWatch metrics); essential `false` (sidecar failure must not kill the app container); CPU 256 / memory 512; no inbound ports needed; add `logConfiguration` pointing at the API CloudWatch log group with stream prefix `adot`. The API application container must be named `backend` — this name is referenced by `deploy-live-backend.yml` when patching the image in the task definition (see the `jq` filter `.name == "backend"` in the deploy workflow). |
...
| `aws_appautoscaling_policy` (API — ALB request count) | **missing** | `policy_type = "TargetTrackingScaling"`; `predefined_metric_type = "ALBRequestCountPerTarget"`; `target_value = 1000`; `resource_label` must be set to `"${aws_lb.main.arn_suffix}/${aws_lb_target_group.api.arn_suffix}"` — this is the required format for `ALBRequestCountPerTarget` and the most commonly missed field |
...
| `aws_flow_log` | **missing** | `traffic_type = "ALL"`; `log_destination_type = "s3"`; `log_destination = "arn:aws:s3:::${var.regional_logging_bucket_name}/flow-logs/"`; `log_format` can be left unset to use the AWS default format (includes srcaddr, dstaddr, srcport, dstport, protocol, action, bytes, packets — sufficient for network audit). No IAM role needed for S3 delivery — the bucket policy's `delivery.logs.amazonaws.com` principal handles authorization. |
...
| `aws_iam_role` (task role — worker) | **missing** | Secrets Manager `GetSecretValue` for MongoDB URI and LLM API keys (no Redis auth token — the worker does not connect to ElastiCache); `xray:PutTraceSegments` + `xray:PutTelemetryRecords` for ADOT sidecar (same as API task role); `cloudwatch:PutMetricData` scoped to a custom namespace (e.g. `Buddy360/Worker`) for publishing MongoDB pending job count; no SQS or Step Functions permissions needed |
...
| `aws_cloudtrail` | **missing** | `is_multi_region_trail = false` (regional trail for ap-south-1 only); `enable_log_file_validation = true`; `include_global_service_events = false`; `kms_key_id = aws_kms_key.cloudtrail.arn`; `s3_bucket_name = var.regional_logging_bucket_name`; `s3_key_prefix = "cloudtrail"`; `event_selector { read_write_type = "All"; include_management_events = true }` |
| `aws_kms_key` + `aws_kms_alias` | **missing** | `enable_key_rotation = true`; `deletion_window_in_days = 30`. **Key policy must include two statements** — (1) allow the account root to manage the key: `Principal = {"AWS": "arn:aws:iam::${data.aws_caller_identity.current.account_id}:root"}`, `Action = "kms:*"`; (2) allow CloudTrail to use the key: `Principal = {"Service": "cloudtrail.amazonaws.com"}`, `Action = ["kms:GenerateDataKey*", "kms:DescribeKey"]`, `Condition = {"StringEquals": {"aws:SourceArn": "arn:aws:cloudtrail:ap-south-1:${account_id}:trail/${trail_name}"}}`. Without statement (2), CloudTrail cannot encrypt logs and the trail creation succeeds but log delivery silently fails. |
...
| `aws_sns_topic_subscription` (HTTPS) | **missing** | PagerDuty endpoint on prod only |
...
> variable "pagerduty_webhook_url"        { default = "" }
...
| `aws_iam_role_policy` (task role — API, S3 uploads) | **missing** | `s3:PutObject` + `s3:GetObject` scoped to uploads bucket ARN; the API task (server-side) generates the pre-signed URL using the task role credentials and returns it to the browser — no additional IAM action is needed beyond `s3:PutObject` |
...
| Terraform resource | Status | Notes |
|---|---|---|
| `aws_s3_bucket_policy` (regional logging) | **missing** | Required for three principals: (1) ALB access logs — grants `s3:PutObject` to the ELB service account for ap-south-1 (account ID 718504428378); (2) CloudTrail — grants `s3:GetBucketAcl` + `s3:PutObject` to `cloudtrail.amazonaws.com`; (3) VPC Flow Logs — grants `s3:PutObject` to `delivery.logs.amazonaws.com` with condition `aws:SourceAccount` scoped to your account ID (`AWSLogDeliveryWrite` + `AWSLogDeliveryAclCheck`). Reference bucket via `var.regional_logging_bucket_name`. Without the Flow Logs principal, VPC Flow Logs silently fail to write. |
...
| `aws_kinesis_firehose_delivery_stream` | **missing** | Must be in us-east-1; name must start with `aws-waf-logs-`. `s3_configuration`: `bucket_arn = "arn:aws:s3:::${var.global_logging_bucket_name}"`; `prefix = "waf-logs/"`; `error_output_prefix = "waf-logs-errors/"`; `buffering_interval = 300` (5 min); `buffering_size = 5` (MB); `compression_format = "GZIP"`. |
...
| `aws_cloudtrail` (us-east-1) | **missing** | Covers CloudFront + WAF management events; destination: global S3 logging bucket |
...
| `mongodbatlas_cloud_backup_schedule` | **missing** | 30-day retention for prod |
...
| Atlas PrivateLink (full 7-phase sequence) | ✗ skip — use public SRV URI directly; run terraform-atlas once (Phase 1 only), skip the `aws_vpc_endpoint` handshake and Phase 5 | ✓ full sequence | ✓ full sequence |
| Atlas Termination Protection (console toggle) | ✗ | ✓ | ✓ |
| NAT EIP fallback in IP access list | ✓ (NAT is primary path; no PrivateLink) | ✓ | ✓ |

> **dev shortcut:** because PrivateLink is skipped, the `vpc_endpoint_id` variable stays `""` permanently. terraform-atlas only ever runs once (Phase 1). Skip Phases 2 → 5 entirely; the public SRV URI is seeded into Secrets Manager at terraform-live-backend apply and never rotated.
...
> **dev:** single-node, no cluster mode, no failover. Significant cost saving (~$0.017/hr vs ~$1.01/hr for prod). No code path changes needed — the Redis client works identically against both topologies.
...
| `aws_guardduty_detector` (us-east-1) | ✗ | optional | ✓ |
...
| PagerDuty SNS subscription (HTTPS) | ✗ (`pagerduty_webhook_url = ""`) | ✗ | ✓ (set `PAGERDUTY_WEBHOOK_URL` secret) |
| X-Ray sampling rules | 1 rule, `fixed_rate = 0.05` (5%) | 1 rule, `fixed_rate = 0.05` | 2 rules: baseline 1% + errors 100% |
| `aws_guardduty_detector` (ap-south-1) | ✗ skip | optional | ✓ |
| `aws_cloudtrail` (ap-south-1) | ✗ skip | ✓ | ✓ |
| `aws_kms_key` (CloudTrail encryption) | ✗ skip — use default S3 SSE | ✓ | ✓ |
...
# Skip CloudTrail (ap-south-1) + KMS on dev
variable "enable_cloudtrail" { default = true }
...
# PagerDuty SNS subscription — prod only (driven by secret, not boolean flag)
variable "pagerduty_webhook_url" { default = "" }
...
resource "mongodbatlas_advanced_cluster" "main" {
  ...
  backup_enabled = var.backup_enabled  # was hardcoded true
  lifecycle {
...
```hcl
# Skip WAF association on dev (CloudFront uses default_action = allow)
...
| `enable_flow_logs` | infra-live-backend | `false` | `false` | `true` |
...
> **Environment shortcuts:** this sequence describes the full prod path. For **dev**, skip Phases 1, 2 (Atlas VPC endpoint only), 5, and 6 — run terraform-live-backend with `atlas_endpoint_service_name = ""` (PrivateLink skipped via `count=0`), seed Secrets Manager with the public Atlas SRV URI, and stop after Phase 4. For **stg**, run the full sequence but with stg-sized resources as documented in the environment matrix.
...
| 0.7 | Create **S3 regional logging bucket** (ap-south-1) | Block all public access; set lifecycle: Glacier after 30 days, expire after 365 days. Note the bucket name. |
| 0.8 | Create **S3 global logging bucket** (us-east-1) | Set Object Ownership to `BucketOwnerPreferred`; block all public access; set lifecycle rules. Note the bucket name. |
...
| `MONGODB_URI` | Atlas public SRV connection string; used only to seed Secrets Manager on first apply — the Secrets Manager value is updated to the private endpoint URI after Phase 5 by `terraform-atlas` directly; this GitHub secret is never changed after initial setup | terraform-live-backend |
...
| `PAGERDUTY_WEBHOOK_URL` | PagerDuty HTTP integration URL for the SNS HTTPS subscription (prod only). Obtain from PagerDuty: Integrations → Add Integration → AWS CloudWatch (or Generic Webhook). Leave blank for dev/stg — `aws_sns_topic_subscription` (HTTPS) should only be created when this value is non-empty (guard with `count = var.pagerduty_webhook_url != "" ? 1 : 0` in the resource). | terraform-live-backend |
...
| `mongodbatlas_cloud_backup_schedule` | 30-day continuous backup schedule |
...
- ElastiCache cluster mode (**~5–10 min Redis outage** — destroy + create)
- IAM roles + policies, Secrets Manager (initialised from GitHub secrets on first apply only)
- CloudWatch alarms + dashboard, GuardDuty (ap-south-1), CloudTrail + KMS
...
> - **Redis destroy + create (~5–10 min outage):** The ElastiCache replication group is replaced in the same apply. All in-flight cache reads/writes fail during this window. Ensure the API returns results on cache misses (do not 500) **before running this phase on prod.**
...
/<APP_NAME>/<ENV>/backend/<REGION>/ecs_service_name       →  <service-name>
...
`terraform-live-edge` (Phase 3) reads `alb_internal_fqdn`. `terraform-atlas` (Phase 5) reads `atlas_vpc_endpoint_id`, `nat_eip_addresses`, and `secrets_manager_arn`.
...
| MongoDB traffic via PrivateLink | VPC Flow Logs — port 27017 traffic goes to VPC endpoint ENI, not NAT GW |
| Redis cluster mode active | ElastiCache console — cluster mode enabled, 3 shards |
...
| Route 53 hosted zone | NAT Gateways + EIPs |
...
resource "mongodbatlas_advanced_cluster" "main" {
  ...
  replication_specs {
    region_configs {
      electable_specs {
        instance_size = var.atlas_instance_size  # M10 (dev) / M20 (stg) / M50 (prod)
        node_count    = 3
      }
      provider_name = "AWS"
  ...
  backup_enabled = var.backup_enabled  # false (dev) / true (stg + prod)

  lifecycle {
    prevent_destroy = true
    # Hard cap — prevent Terraform from upgrading instance size beyond what was imported.
    ignore_changes = [replication_specs[0].region_configs[0].electable_specs[0].instance_size]
  }
}
...
resource "mongodbatlas_cloud_backup_schedule" "main" {
  ...
  restore_window_days      = var.restore_window_days  # 0 (dev) / 7 (stg) / 30 (prod)
}
...
output "endpoint_service_name" {
  ...
  value = var.enable_privatelink ? mongodbatlas_privatelink_endpoint.main[0].endpoint_service_name : ""
}
...
# Verify this attribute path against the mongodbatlas provider version in use —
# it may be connection_strings[0].privateEndpoint[0].srvConnectionString in older versions.
    ...
    steps:
      ...
      - name: Resolve SSM inputs
        ...
        run: |
          ...
          # NAT EIP addresses — empty on Phase 1 run; populated after Phase 2
...
**2. TTL index for automatic document expiry** — without this the `jobs` collection grows indefinitely. Each completed job document holds the full LLM response; at 1M users this becomes significant storage quickly.
...
// After POST /api/chat returns job_id, start polling
...
> **Timeout UX note:** When `pollJob` throws a `TimeoutError`, display a non-dismissable banner ("Your response is still being generated — refresh this page to check") rather than an error state. The worker will continue processing and the result will be readable on the next page load. Do not surface `"Timed out"` as a failure to the user.
...
```
React (nginx on :5173) → POST /api/chat
...
- **Atlas M50**: ~$1.04/hr (hard cap; no M60 upgrade path)
- **NAT Gateway**: $0.045/hr endpoint + $0.045/GB processed
- **VPC Interface Endpoints**: $0.01/AZ/hr per service
- 10M-user column assumes ~10× traffic, 20 ECS tasks average, Atlas M50 (hard cap), 6 Redis shards × 2 nodes (requires one manual destroy+create before reaching 10M — schedule a maintenance window)
