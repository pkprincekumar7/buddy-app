...
> For full per-service cost breakdowns, hourly rates, and pricing basis see [aws-cost-estimate.md](aws-cost-estimate.md).
> Note: that file excludes MongoDB Atlas (external to AWS billing). Atlas costs: M10 ~$58/mo (dev/sbx), M20 ~$144/mo (stg), M30 ~$389/mo (prod ≤100K users), M40 ~$749/mo (1M users), M50 ~$1,440/mo (10M users, hard cap).

---

## Complete AWS service inventory for the target architecture

> For a full per-environment resource inventory see [aws-resources.md](aws-resources.md).
