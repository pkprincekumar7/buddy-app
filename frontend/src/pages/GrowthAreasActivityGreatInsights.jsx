  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const rawGameResults = areaDoc.child_activity?.results;
        if (rawGameResults) {
        ...
        const cached =
          Array.isArray(areaDoc.ai_three_month_recommendations) &&
            ...
            >
              {answered.map((q, i) => (
                ...
                >
                  ...
                  <p className="text-sm font-medium text-white">{interactiveAnswers[q.id]}</p>
                </motion.div>
              ))}
            </motion.div>
          );
