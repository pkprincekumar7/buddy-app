...
const LIFE_PHASE_LABELS: Record<string, string> = {
  foundation: 'Foundation (age 8–11) — building core habits, identity, and learning skills',
  exploration: 'Exploration (age 12–14) — early adolescence, self-discovery, peer influence',
  direction: 'Direction (age 15+) — mid-to-late adolescence, goal setting, future orientation',
};
...
export function buildJourneyRecommendationsPrompt({
  ...
  const hobbies = Array.isArray(childData.hobbies)
    ...
    : 'unknown';
  const childName = typeof childData.name === 'string' ? childData.name : 'the child';
  const gender = typeof childData.gender === 'string' ? childData.gender : 'unknown';
  const lifePhaseLabel = (lifePhase && LIFE_PHASE_LABELS[lifePhase]) ?? lifePhase ?? 'unknown';
...
• School: ${typeof childData.school === 'string' ? childData.school : 'not captured'}
...
2. focus_areas — Exactly four immediate growth focus areas, one per pillar: Mind, Heart, Body, Talent (use these exact pillar labels, no repeats). For each area:
   - Name the pillar and a specific focus topic relevant to ${childName}'s age (${String(age ?? 'unknown')}), gender (${gender}), and personality.
...
3. initial_missions — Exactly three starter weekly missions for ${childName} to begin this week. Each mission must:
   - Be concrete and completable within one week by a ${String(age ?? 'unknown')}-year-old.
...
export function buildGoalsMonthlyPlanPrompt({
  ...
  return `Create a focused 3-month goal plan for ${childName ?? 'the child'}, age ${childAge ?? 'unknown'}, gender ${childGender ?? 'unknown'}.

Important: All activity titles, objectives, and language must be calibrated to be age-appropriate for a ${childAge ?? 'unknown'}-year-old ${childGender ?? ''} child. Younger children (8–10) need simpler, play-based, concrete tasks. Older children (13+) can handle more abstract, self-reflective, and goal-oriented activities. Do not use generic titles — make every activity feel relevant to this specific child's age and interests.
...
Child name: ${childName ?? 'the child'}${personaLine ? `\nChild profile: ${personaLine}` : ''}${goalLine ? `\n${goalLine}` : ''}${impactLine ? `\n${impactLine}` : ''}
...
Activity objective: "${objective}"${profileLine ? `\nChild profile: ${profileLine}` : ''}${isFollowUp ? '\nContext: This is a follow-up (Week 3 & 4) activity — a direct progression of a previous attempt.' : ''}${originalContext}
...
export function buildProgressComparisonPrompt({
  ...
  return `Compare two assessments for the same child activity objective and determine honestly whether the child improved, declined, or stayed the same.${profileLine ? `\nChild profile: ${profileLine}` : ''}
...
- Child's answers: ${originalAnswersText ? `\n${originalAnswersText}` : '(not available)'}
...
- Child's answers: ${followupAnswersText ? `\n${followupAnswersText}` : '(not available)'}
