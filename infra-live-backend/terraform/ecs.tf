  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        { name = "POSTGRES_HOST", value = data.terraform_remote_state.live_db.outputs.rds_endpoint },
        { name = "POSTGRES_PORT", value = tostring(data.terraform_remote_state.live_db.outputs.rds_port) },
        ...
        { name = "POSTGRES_MAX_OVERFLOW", value = tostring(var.postgres_max_overflow) },
        { name = "DEFAULT_REGION", value = var.default_region },
        { name = "REGIONAL_DB_URLS", value = "{}" },
        { name = "RECONCILER_INTERVAL_MINUTES", value = tostring(var.reconciler_interval_minutes) },
      ...
      secrets = [
        { name = "POSTGRES_USER", valueFrom = "${data.terraform_remote_state.live_db.outputs.rds_secret_arn}:username::" },
        { name = "POSTGRES_PASSWORD", valueFrom = "${data.terraform_remote_state.live_db.outputs.rds_secret_arn}:password::" },
