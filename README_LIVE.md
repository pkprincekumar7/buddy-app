...
This guide covers the four Terraform modules and six GitHub Actions workflows that provision and deploy the production-grade infrastructure for buddy360.
...
|---|---|---|---|
| Bucket | `infra-live-bucket/` | `terraform-live-bucket.yml` | Import pre-existing S3 buckets; publish names/ARNs to SSM |
...
```
Written by infra-live-bucket:
  /{APP_NAME}/{env}/bucket/frontend_name                    — frontend S3 bucket name (global)
  /{APP_NAME}/{env}/bucket/frontend_arn                     — frontend S3 bucket ARN  (global)
  /{APP_NAME}/{env}/bucket/backend_name                     — backend S3 bucket name  (us-east-1, shared)
  /{APP_NAME}/{env}/bucket/backend_arn                      — backend S3 bucket ARN   (us-east-1, shared)
...
```
infra-live-backend   reads: /bucket/backend_name  and  /bucket/backend_arn
infra-live-edge      reads: /backend/{backend_region}/alb_internal_fqdn
                            /bucket/frontend_name  and  /bucket/frontend_arn
...
| Frontend assets bucket | `person-frontend-{env}-bucket` | `us-east-1` | One per environment; imported by `infra-live-bucket` |
| Backend app bucket | `person-backend-{env}-app-bucket` | `us-east-1` | One per environment; shared across backend regions; imported by `infra-live-bucket` |
...
| ACM certificate | covers internal subdomain | same as backend | For ALB HTTPS; set as `ACM_CERTIFICATE_ARN_AP_SOUTH_1` (or EU/US variant) |
...
| `ACM_CERTIFICATE_ARN_US_EAST_1` | `terraform-live-edge` | ACM cert in `us-east-1` for CloudFront |
| `ACM_CERTIFICATE_ARN_AP_SOUTH_1` | `terraform-live-backend` | ACM cert in `ap-south-1` for ALB |
...
| `CORS_ORIGINS` | `terraform-live-backend` | Injected into Secrets Manager |
| `COOKIE_DOMAIN` | `terraform-live-backend` | Injected into Secrets Manager |
...
```
Step 1  terraform-live-bucket   action=apply
        → imports S3 buckets (us-east-1), writes SSM params

Step 2  terraform-live-backend  action=apply  aws_region={backend-region}
        → VPC / ECS / ALB / Redis live in chosen region
        → internal DNS record + SSM written
        → backend API reachable once edge is deployed ✓

Step 3  terraform-live-edge     action=apply  backend_region={backend-region}
        → CloudFront + WAF + public DNS live
        → backend API accessible via Postman immediately ✓

Step 4  terraform-live-frontend action=apply
        → S3 bucket policy applied

Step 5  deploy-live-backend     aws_region={backend-region}
        → Docker image pushed, ECS updated

Step 6  deploy-live-frontend
...
Deploy steps 1–2 and 5 independently per region. Steps 3–4 and 6 are global (run once). To add a second region after initial deploy:
...
**Step 1 — infra-live-bucket**
- Imports pre-existing S3 buckets into Terraform state (`prevent_destroy = true`)
- Writes frontend bucket name/ARN to SSM (global, no region in path)
- Writes backend bucket name/ARN to SSM (no region in path — one bucket shared by all backend regions)

**Step 2 — infra-live-backend**
...
Step 4  terraform-live-bucket   action=destroy  aws_region={region}
                                                → /bucket/* SSM params deleted
                                                  ⚠ bucket resources: see caveat below
```

Repeat steps 3–4 for each deployed region before destroying edge.

### infra-live-bucket destroy caveat

The S3 bucket resources have `lifecycle { prevent_destroy = true }`. Running `terraform destroy` will **error** on them intentionally — the buckets are pre-existing and must never be deleted by Terraform.

To fully clean up state after a destroy:

```bash
cd infra-live-bucket/terraform
terraform state rm aws_s3_bucket.frontend
terraform state rm aws_s3_bucket.backend
terraform destroy   # now only destroys the SSM parameters
```
...
If a GitHub secret is not set the value falls back to `REPLACE_ME` (or `""` for `COOKIE_DOMAIN`).
...
aws secretsmanager put-secret-value \
  ...
  --secret-string '{
    ...
    "GEMINI_API_KEY":     "AIza...",
    "CORS_ORIGINS":       "https://buddy-dev.learning-dev.com",
    "COOKIE_DOMAIN":      "",
...
```
terraform-state-files/{APP_NAME}/{env}/bucket/{region}/terraform.tfstate
...
Each backend region has its own state file. Bucket, edge, and frontend state files use `us-east-1` as the region component.
