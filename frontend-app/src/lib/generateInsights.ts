...
function buildInsightsPrompt(
  ...
  // Child profile
  lines.push(
    `You are generating personalised progress insights for a parent about their child.`,
  );
  lines.push('');
  ...
  monthData.forEach(({ month, pairs }) => {
    ...
    lines.push(
      `Objective: ${String(
        (month as Record<string, unknown>).objective ?? '',
      )}`,
    );
    ...
    pairs.forEach((pair, pIdx) => {
      ...
      if (orig?.completed) {
        ...
        if (orig.what_changed)
          lines.push(`    What changed: ${orig.what_changed}`);
        if (orig.what_learned)
          lines.push(`    What was learned: ${orig.what_learned}`);
        if (orig.recommendation)
          lines.push(`    Recommendation given: ${orig.recommendation}`);
        lines.push(`    AI Feedback: ${orig.ai_feedback ?? 'none'}`);
        if (orig.parent_feedback)
          lines.push(`    Parent's note: ${orig.parent_feedback}`);
      } else {
        ...
        if (fu.scorable !== false) {
          ...
          lines.push(
            `    Progress vs original: ${
              fu.progress_observation ?? 'No Improvement'
            }`,
          );
        }
        if (fu.what_changed) lines.push(`    What changed: ${fu.what_changed}`);
        if (fu.what_learned)
          lines.push(`    What was learned: ${fu.what_learned}`);
        if (fu.recommendation)
          lines.push(`    Recommendation given: ${fu.recommendation}`);
        lines.push(`    AI Feedback: ${fu.ai_feedback ?? 'none'}`);
        if (fu.parent_feedback)
          lines.push(`    Parent's note: ${fu.parent_feedback}`);
      } else {
  ...
  lines.push(
    `Rules: ` +
      ...
      `Language should be clear and specific. Warm tone is welcome but must not override honesty. Age-appropriate for a ${
        childAge ? String(childAge) + '-year-old' : 'child'
      }.`,
  );
