...
export const personalityCategories: Record<
  string,
...
export const personalityTypes: Record<string, PersonalityTypeEntry> = {
  Ambitious: {
    ...
    description:
      '{childName} sets high standards, aims big, and is motivated by achieving success.',
    famous_people: [
      {
  ...
  Determined: {
    ...
    description:
      '{childName} shows strong persistence and is motivated to finish what they start.',
    famous_people: [
      {
        name: 'Thomas Edison',
        image:
  ...
  Outgoing: {
    ...
    description:
      '{childName} thrives in social settings and energizes others through their presence.',
    famous_people: [
      {
        name: 'Oprah Winfrey',
        image:
  ...
  Creative: {
    ...
    description:
      '{childName} enjoys creating, imagining new possibilities, and finding unique solutions.',
    famous_people: [
      {
  ...
  Enthusiastic: {
    ...
    description:
      '{childName} approaches new experiences with eagerness and brings energy to their surroundings.',
    famous_people: [
      {
        name: 'Robin Williams',
        image:
  ...
  Restless: {
    ...
    description:
      '{childName} prefers variety and fast-paced activities and seeks new experiences.',
    famous_people: [
      {
  ...
  'Highly Energetic': {
    ...
    description:
      '{childName} has a high energy level and can engage in multiple activities with stamina.',
    famous_people: [
      {
        name: 'Serena Williams',
        image:
  ...
  Thinker: {
    ...
    description:
      '{childName} enjoys thinking deeply, solving problems, and asking questions.',
    famous_people: [
      {
  ...
  Playful: {
    ...
    description:
      '{childName} brings fun and joy to situations and approaches life with a light-hearted spirit.',
    famous_people: [
      {
        name: 'Jim Carrey',
        image:
...
function roleModelAvatars(
  roleModels: unknown,
  fallbackName: string | undefined,
): FamousPerson[] {
  const list = Array.isArray(roleModels)
    ? (roleModels as Array<Record<string, unknown>>)
    : [];
  const two = [...list.slice(0, 2)];
  while (two.length < 2)
    two.push({
      name: two.length === 0 ? fallbackName ?? 'Role model A' : 'Role model B',
    });
  return two.map(r => {
    const nm =
      typeof r?.name === 'string' && r.name.trim() ? r.name.trim() : 'Guide';
    return { name: nm, image: generateAvatarDataUri(nm) };
  });
}
