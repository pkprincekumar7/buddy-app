...
import { GradientIconBox, GradientSurface } from '@/components/shared/GradientView';
...
const monthColors: MonthColor[] = [
  { gradFrom: '#0d9488', gradTo: '#14b8a6', objectiveStrip: 'bg-teal-500/10 border-teal-500/25', labelText: 'text-teal-400', dotBg: 'bg-teal-500' },
  { gradFrom: '#2563eb', gradTo: '#3b82f6', objectiveStrip: 'bg-blue-500/10 border-blue-500/25', labelText: 'text-blue-400', dotBg: 'bg-blue-500' },
  { gradFrom: '#9333ea', gradTo: '#a855f7', objectiveStrip: 'bg-purple-500/10 border-purple-500/25', labelText: 'text-purple-400', dotBg: 'bg-purple-500' },
];
...
function ActivityCardIcon({
  ...
  return (
    <View className={`mt-0.5 h-5 w-5 flex-shrink-0 items-center justify-center rounded-full ${dotBg}`}>
      <Text className="text-[10px] font-bold text-white">{index + 1}</Text>
    </View>
  );
}

function AccordionContent({ isOpen, children }: { isOpen: boolean; children: ReactNode }) {
  ...
  const handleLayout = useCallback((e: LayoutChangeEvent) => {
    ...
    if (animHeight.value >= 9999) {
      ...
      animHeight.value = withTiming(h, { duration: 200, easing: Easing.out(Easing.ease) });
    }
  }, [animHeight]);
        ...
        >
          <View className="flex-row items-center gap-3 flex-1 mr-2">
            ...
            <View className="flex-1">
              ...
              <Text className="font-bold text-white" numberOfLines={2}>{month.goal}</Text>
            </View>
          </View>
      ...
      <AccordionContent isOpen={!!isOpen}>
        <View className="px-5 py-5 gap-5">
          {month.periods?.map((period, pIdx) => (
            <View key={`${idx}-${pIdx}`}>
              <Text className={`mb-3 text-xs font-bold uppercase tracking-widest ${color.labelText}`}>
                {period.label}
              </Text>
              <View className="gap-2.5">
                {period.activities?.map((act, aIdx) => {
                  ...
                  if (act.completed) cardClass = 'border border-emerald-500/20 bg-emerald-500/10';
                  else if (isLocked) cardClass = 'border border-dashed border-slate-700 bg-slate-900';
                  ...
                  return (
                    ...
                    >
                        ...
                        {act.completed ? (
                          <View className="mt-2 gap-1">
                            <Text className="text-xs font-semibold text-emerald-400">
                              ✅ {String((act.ai_feedback as string | number | boolean | null | undefined) ?? '')}
                            </Text>
                            {act.scorable !== false ? (
                              <Text className="text-xs font-bold text-slate-300">
                                Score: {String((act.score as string | number | boolean | null | undefined) ?? '')}/10
                              </Text>
                            ) : (
                              <Text className="text-xs font-bold text-slate-300">
                                Note: {String((act.note as string | number | boolean | null | undefined) ?? '')}
                              </Text>
                            )}
                            {!!act.parent_feedback && (
                          ...
                          >
                            <Text className={`mt-1.5 text-xs font-medium ${color.labelText}`}>
                      ...
                      {act.completed && (
                        <Pressable
                          onPress={() => { void onResetActivity(idx, pIdx, aIdx); }}
...
interface ModalErrorBoundaryState { hasError: boolean; }
interface ModalErrorBoundaryProps { children: ReactNode; }
...
export default function GoalsDashboardScreen() {
  ...
  return (
    ...
    <View className="flex-1 bg-background">
      <ScrollView ref={scrollRef} className="flex-1" contentContainerStyle={{ paddingHorizontal: 16, paddingVertical: 40 }}>
        {/* Header */}
        <View className="mb-8 items-center">
          <View className="mb-4">
            <GradientIconBox from="#2dd4bf" to="#0d9488" size={56} radius={16}>
          ...
          <Text className="text-slate-400 text-center">Personalized goals powered by Buddy360</Text>
        ...
        {isLoading ? (
          <View className="items-center justify-center py-24 gap-4">
            ...
            />
            <Text className="text-slate-500">Building your 3-month plan...</Text>
          </View>
        ) : (
          ...
          <GoalPlanContext.Provider value={contextValue}>
            <View>
              ...
              <View className="items-center pt-2 mb-4">
                ...
                >
                  <Text className="text-[#0a0a0a] font-semibold">View Progress And Insights</Text>
                </Button>
              </View>
                ...
                right={
                  ...
                  >
                    <View className="flex-row items-center gap-2">
                      ...
                      <Text className="text-slate-300">Regenerate Plan</Text>
                    </View>
                  </Button>
                }
              />
