...
type PersonalityTypeRouteProp = RouteProp<{ PersonalityType: { childId?: string } | undefined }, 'PersonalityType'>;
...
export default function PersonalityTypeScreen() {
  ...
  const [status, setStatus] = useState<'loading' | 'analysing' | 'ready' | 'error'>('loading');
    ...
    void (async () => {
      try {
        ...
        try {
          ...
          console.warn('[PersonalityTypeScreen] LLM failed, falling back to rule-based:', err);
  ...
  return (
    <View style={{ flex: 1 }}>
      <Animated.View style={contentStyle} className="flex-1 bg-background">
        ...
        >
          <PersonalityAnalysis mbtiResult={mbtiResult} childName={childName} ready={!showSplash} />
          ...
          <PageActions
            ...
            right={
              ...
              >
                <View className="flex-row items-center gap-1.5">
                  <Text className="text-sm font-semibold text-[#0a0a0a]">Continue</Text>
