...
function suggestedActivitiesFromGameRecommendations(rec: unknown): string[] {
  ...
  return Array.isArray(n.suggested_activities)
    ? (n.suggested_activities as string[])
    : [];
}
...
const areaQuestions: Record<string, AreaQuestion[]> = {
  life_ambition: [
    {
      ...
      followUp:
        "That's wonderful! Dreams are the seeds of future achievements.",
    },
    {
    ...
    },
    {
      id: 'explore_options',
      question:
        'Do you think {name} should explore other career options as well?',
      type: 'choice',
      options: ['Yes', 'No', 'Not sure at this point'],
  ...
  self_care: [
    ...
    },
    {
      id: 'stress_response',
      question:
        'How does {name} typically respond when stressed or overwhelmed?',
      type: 'text',
      placeholder: 'e.g., withdraws, cries, talks about it...',
      followUp:
        'Understanding stress responses helps us build better coping strategies.',
    },
    {
      id: 'sleep_habits',
      ...
      followUp:
        'Good sleep is fundamental to emotional and physical well-being.',
    },
    {
      id: 'self_soothing',
      ...
      followUp:
        'Self-soothing skills are important tools for lifelong wellness.',
    },
    {
      id: 'self_care_goals',
  ...
  critical_thinking: [
    {
      id: 'problem_approach',
      question:
        "How does {name} typically approach a problem they can't solve immediately?",
      type: 'choice',
      options: [
    ...
    },
    {
      id: 'decision_making',
      question:
        'Can {name} make decisions independently, weighing pros and cons?',
      type: 'choice',
    ...
    },
    {
      id: 'thinking_goals',
      question:
        'What critical thinking skill would you most like {name} to strengthen?',
      type: 'text',
      placeholder:
        'e.g., logical reasoning, creative solutions, evaluating information...',
  ...
  creativity: [
    {
      ...
      followUp:
        'Wonderful! Creative outlets are essential for expression and growth.',
    },
    {
      id: 'imagination_use',
      question:
        'How often does {name} engage in imaginative play or storytelling?',
      type: 'choice',
    ...
    },
    {
      ...
      question:
        'Does {name} feel confident sharing their creative work with others?',
      type: 'choice',
      options: [
        'Very confident',
        'Somewhat confident',
    ...
    },
    {
      id: 'open_ended_play',
      question:
        'Does {name} prefer structured activities or open-ended creative play?',
      type: 'choice',
      options: [
        'Prefers structured',
        'Prefers open-ended',
    ...
    },
    {
      id: 'creativity_goals',
      question:
        "How would you like to nurture {name}'s creativity in the next 3 months?",
      type: 'text',
  ...
  physical_wellness: [
    ...
    },
    {
      id: 'body_awareness',
      question:
        "Is {name} aware of their body's signals (hunger, tiredness, discomfort)?",
      type: 'choice',
    ...
    },
    {
      ...
      followUp:
        'Balancing screen time with physical activity is a key wellness goal.',
    },
    {
      id: 'wellness_goals',
      question:
        'What physical wellness goal would you set for {name} over the next 3 months?',
      type: 'text',
  ...
  social_skills: [
    ...
    },
    {
      ...
      question:
        'How does {name} behave in group settings (school, teams, clubs)?',
      type: 'choice',
      options: [
        'Natural leader',
        'Active participant',
      ...
      followUp:
        'Understanding group dynamics helps us tailor the right activities.',
    },
    {
      id: 'social_goals',
      question:
        'What social skill would you most like {name} to build in the next 3 months?',
      type: 'text',
      placeholder:
        'e.g., starting conversations, teamwork, expressing feelings...',
...
function buildAreaProgressPayload(args: {
  ...
  const interactive_draft =
    cq?.type === 'text'
      ? { question_id: cq.id, text: currentAnswer ?? '' }
      : null;
  ...
  return {
    ...
    child_activity: childGameResults
      ? {
          selections: Array.isArray(childActivitySelections)
            ? childActivitySelections
            : [],
...
function deriveInteractiveUiFromProgress(
  ...
  const mergedAnswers = answersForArea(area.id, {
    ...fromCompleted,
    ...rawProgress,
  });
  ...
  const hasAnswersForArea = qs.some(q =>
    answerLooksFilled(mergedAnswers[q.id]),
  );
  const blobBelongsToArea =
    areaMatchesPersisted || hasAnswersForArea || completedHasArea;
  ...
  const firstIncomplete = qs.findIndex(
    q => !answerLooksFilled(mergedAnswers[q.id]),
  );
  ...
  if (
    ...
    typeof p.interactive_step === 'number' &&
    qs.length > 0
  ) {
    interactiveStepIx = Math.max(
      0,
      Math.min(qs.length - 1, p.interactive_step),
    );
  }
...
function applyTileEntryInteractivePreference(
  ...
  if (d.step === 'activity_summary' && anyFilled) {
    const cq = qs[0];
    const caUse =
      cq?.type === 'text' && answerLooksFilled(d.mergedAnswers[cq?.id ?? ''])
        ? String(d.mergedAnswers[cq.id])
        : '';
    return {
      step: 'interactive_activity',
      interactiveStep: 0,
  ...
  return {
    step: d.step,
    interactiveStep: d.interactiveStep,
...
interface PhaseState {
  ...
  mergeChildGameFromServer: (
    areaId: string,
    opts?: { reopenGame?: boolean },
  ) => Promise<void>;
  generateAiRecommendations: (
    childResults: ChildGameResults | null,
  ) => Promise<void>;
                ...
                try {
                  ...
                  const dbRecs =
                    ...
                    p.ai_three_month_recommendations.length > 0
                      ...
                      : extractAiRecommendationsFromCompleted(
                          completedDocs,
                          area.id,
                        );
                  const airMerged =
                  ...
                  if (isInProgress) {
                    ...
                    if (ca2) {
                      setChildActivitySelections(
                        Array.isArray(ca2.selections)
                          ? (ca2.selections as string[])
                          : [],
                      );
                  ...
                  if (ca3) {
                    setChildActivitySelections(
                      Array.isArray(ca3.selections)
                        ? (ca3.selections as string[])
                        : [],
                    );
                  } else {
                    setChildActivitySelections(
...
function InteractiveActivityScreen({ ps }: { ps: PhaseState }) {
  ...
  const handlePreviousQuestion = () => {
    ...
    const savedAns = prevQuestion
      ? interactiveAnswers[prevQuestion.id]
      : undefined;
    setCurrentAnswer(
      prevQuestion?.type === 'text' && typeof savedAns === 'string'
        ? savedAns
        : '',
    );
            ...
            <Button
              onPress={() => {
                void (async () => {
                  ...
                  await mergeChildGameFromServer(selectedArea.id, {
                    reopenGame: true,
                  });
                  setParentLiked(true);
                })();
          ...
          <ChildActivityGame
            ...
            onComplete={async results => {
              ...
              const child_activity = {
                selections: Array.isArray(results.selections)
                  ? [...results.selections]
                  : [],
              ...
              try {
                ...
                console.error(
                  '[RecommendationsPhase] Could not save game results:',
                  err,
                );
                toast.error(
...
export default function RecommendationsPhase({
  ...
  const [selectedActivity, setSelectedActivity] = useState<ActivityDef | null>(
    null,
  );
  const [parentLiked, setParentLiked] = useState<boolean | null>(null);
  const [wantChildActivity, setWantChildActivity] = useState<boolean | null>(
    null,
  );
  ...
  const [childGameResults, setChildGameResults] =
    useState<ChildGameResults | null>(null);
  ...
  const debouncedSaveAreaProgress = useMemo(
    () =>
      debounce((payload: Record<string, unknown>) => {
        api.completedGrowthAreas
          .append(activeChildId ?? '', payload)
          .catch(() => {});
      }, 400),
    [activeChildId],
  );
    ...
    void (async () => {
      try {
        ...
        if (completedDocs.length > 0)
          completedAreaIdsRef.current = new Set(
            completedDocs.map(a => a.area_id as string),
          );

        const inProgressDoc =
          allDocs.find(a => a.status === 'in_progress') ?? null;
        if (!inProgressDoc?.area_id) return;

        const areaObj =
          growthAreas.find(a => a.id === inProgressDoc.area_id) ?? null;
        const areaIdx = growthAreas.findIndex(
          a => a.id === inProgressDoc.area_id,
        );
        ...
        if (areaObj) {
          ...
          const ca = p.child_activity as
            ...
            | undefined;
          if (ca) {
            const sels = Array.isArray(ca.selections)
              ? (ca.selections as string[])
              : [];
          ...
          let aiRec =
            ...
            p.ai_three_month_recommendations.length > 0
              ? p.ai_three_month_recommendations
              : null;
          aiRec ??= extractAiRecommendationsFromCompleted(
            completedDocs,
            areaObj.id,
          );
  ...
  const saveCompletedGrowthArea = useCallback(
    async (
      area: AreaDef,
      answers: Record<string, unknown>,
      recs: unknown[] | null,
  ...
  const mergeChildGameFromServer = useCallback(
    async (areaId: string, { reopenGame }: { reopenGame?: boolean } = {}) => {
      if (!areaId) return;
      try {
        debouncedSaveAreaProgress.flush?.();
        const completedDataRaw = await api.completedGrowthAreas.list(
        ...
        if (ca) {
          setChildActivitySelections(
            Array.isArray(ca.selections) ? (ca.selections as string[]) : [],
          );
          if (!reopenGame) {
            if (ca.results) {
              setChildGameResults(ca.results);
  ...
  const generateAiRecommendations = useCallback(
    async (childResults: ChildGameResults | null) => {
      try {
        const completedDataRaw = await api.completedGrowthAreas.list(
          activeChildId ?? '',
        );
        ...
        console.warn(
          '[RecommendationsPhase] Could not load cached recommendations:',
          err,
        );
      }
      ...
      try {
        ...
        const gr = childResults
          ? normalizeChildGameRecommendations(childResults)
          : null;
        ...
        console.error(
          '[RecommendationsPhase] Failed to generate recommendations:',
          err,
        );
  ...
  return (
    <View className="flex-1">
      ...
      {step === 'child_activity_prompt' && (
        <ChildActivityPromptScreen ps={ps} />
      )}
